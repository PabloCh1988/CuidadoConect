function cargarFechaHoy() {
    ...
    document.getElementById('fecha_hoy').textContent = fechaFormateada;
}
...
async function cargarRutinasHoy() {
    ...
    if (!data.rutinas || data.rutinas.length === 0) {
        ul.innerHTML = `
            <li>
                ...
                <span>
                    <span class="name_user">Sin pendientes</span>
                    <span class="msg_user">Todas las rutinas de hoy están completas.</span>
                </span>
            </li>`;
        return;
    }
    ...
    data.rutinas.forEach(r => {
        ...
        li.innerHTML = `
            ...
            <span>
                <span class="name_user">${r.hora} - ${r.residenteNombre}</span>
                <span class="msg_user">${r.rutinaDescripcion}</span>
                <span class="time_ago">${r.observaciones || ""}</span>
            </span>
        `;
        ul.appendChild(li);
    });
...
async function cargarProximosCumples() {
    try {
        const cumples = await authFetch("residentes/proximos-cumpleanos");

        // if (!cumples.ok) throw new Error("Error al obtener cumpleaños");
        ...
        cumples.forEach(c => {
            ...
            li.innerHTML = `
                <span>
                    <img src="${c.fotoBase64 ? c.fotoBase64 : 'images/layout_img/user.png'}" 
                ...
                <span>
                    <span class="name_user">${c.nombreResidente}</span>
                    <span class="msg_user">
                        Cumple el <b>${new Date(c.fechaCumple).toLocaleDateString()}</b> 
                        – Cumple <b>${c.edadQueCumple}</b> años
                    </span>
                    <span class="time_ago">
                        Faltan <b>${c.diasFaltantes}</b> días
                    </span>
                </span>
            `;
